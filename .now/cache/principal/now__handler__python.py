...
__now_variables = dir(api.principal)
...
if 'handler' in __now_variables or 'Handler' in __now_variables:
    base = api.principal.handler if ('handler' in __now_variables) else  api.principal.Handler
    ...
    if (
        not inspect.iscoroutinefunction(api.principal.app) and
        not inspect.iscoroutinefunction(api.principal.app.__call__)
    ):
        ...
        def now_handler(event, context):
            ...
            response = Response.from_app(api.principal.app, environ)
        ...
        def now_handler(event, context):
            ...
            response = asgi_cycle(api.principal.app, body)
            return response

else:
    print('Missing variable `handler` or `app` in file api.principal.py')
    print('See the docs https://zeit.co/docs/v2/deployments/official-builders/python-now-python')
    exit(1)
