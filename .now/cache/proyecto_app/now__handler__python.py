...
import api.proyecto_app
__now_variables = dir(api.proyecto_app)
...
if 'handler' in __now_variables or 'Handler' in __now_variables:
    base = api.proyecto_app.handler if ('handler' in __now_variables) else  api.proyecto_app.Handler
    ...
    if (
        not inspect.iscoroutinefunction(api.proyecto_app.app) and
        not inspect.iscoroutinefunction(api.proyecto_app.app.__call__)
    ):
        ...
        def now_handler(event, context):
            ...
            response = Response.from_app(api.proyecto_app.app, environ)
        ...
        def now_handler(event, context):
            ...
            response = asgi_cycle(api.proyecto_app.app, body)
            return response

else:
    print('Missing variable `handler` or `app` in file api.proyecto_app.py')
    print('See the docs https://zeit.co/docs/v2/deployments/official-builders/python-now-python')
    exit(1)
